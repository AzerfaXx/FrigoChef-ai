...
import { Plus, Trash2, AlertTriangle, Apple, Beef, Wine, Soup, Search, Package, Clock, ChevronRight, ChevronLeft, Pencil, Check, X, Layers, Calendar } from 'lucide-react';
...
const CATEGORY_ORDER = ['produce', 'meat', 'frozen', 'drinks', 'sauce', 'pantry', 'other'];
...
const Inventory: React.FC<Props> = ({ ingredients, setIngredients }) => {
  ...
  const renderItemCard = (item: Ingredient) => {
    ...
    if (isEditing) {
        return (
            <div key={item.id} className="bg-white dark:bg-slate-800 p-3 rounded-2xl border-2 border-emerald-500/50 shadow-md animate-in fade-in zoom-in-95 mb-3">
    ...
    return (
        <div
        key={item.id}
        className={`group flex items-center justify-between p-3 rounded-2xl border shadow-[0_2px_8px_rgba(0,0,0,0.02)] transition-all duration-300 hover:shadow-md mb-3 ${
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 relative transition-colors duration-300">
      {/* Header */}
      <div className="pt-10 pb-4 px-6 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 flex-shrink-0 z-10 shadow-sm transition-colors duration-300">
        <div className="flex justify-between items-end mb-2">
            ...
            <div className="bg-slate-100 dark:bg-slate-700 p-1 rounded-xl flex items-center gap-1">
                <button 
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1.5 transition-all ${
                        sortMode === 'expiry' 
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-32 scroll-smooth">
        {ingredients.length === 0 ? (
            ...
            <>
                {sortMode === 'expiry' ? (
                     sortedIngredients.map((item) => renderItemCard(item))
                ) : (
                    CATEGORY_ORDER.map((cat) => {
                        ...
                        // Sort items within category by expiry
                        ...
                        return (
                            <div key={cat} className="mb-6 animate-in slide-in-from-bottom-2">
                                <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2 pl-1">
                                    {getCategoryIcon(cat)}
                                    {CATEGORY_LABELS[cat]}
                                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded text-[10px]">{catItems.length}</span>
                                </h3>
                                <div>
                                    {sortedCatItems.map(item => renderItemCard(item))}
                                </div>
                            </div>
                        );
                    })
